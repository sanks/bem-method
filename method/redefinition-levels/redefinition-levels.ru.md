...
Пример показывает сборку для разных страниц проекта:
...
> Изменять исходную реализацию блока можно с помощью [элементов или модификаторов](#Добавление-новых-БЭМ-сущностей-в-проект), добавленных на другом уровне переопределения.

Рассмотрим на примере кнопки (блок `button`):
* Доопределим блок — добавим новое свойство, например, тень.
* Переопределим блок — изменим существующее свойство, например, перекрасим кнопку в другой цвет.
...
Пример CSS-файла показывает, что сначала указывается базовые правила, а потом добавляются особенности с других уровней. 
...
* при необходимости вернуться к первоначальному виду блока, достаточно удалить уровень переопределения.
...
CSS-файл будет создан с помощью импортов в следующем порядке:
...
Результат сборки — стрелкой для страницы `home:
...
Резудьтат сборки — кнопка с новым дизайном:
...
В проекте, поддерживающем разные платформы (например, `mobile` и `desktop`), часть кода описывает общую функциональность и часть, — характерная только для отдельных платформ. Чтобы не копировать общий код для реализации каждой из платформ, используются уровни переопределения: 
