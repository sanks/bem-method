...
В БЭМ-методологии реализация блока складывается из отдельных файлов технологий (`button.css`, `button.js`) — исходных файлов. Все исходные файлы проекта хранятся в группирующих директориях, если говорить в терминах БЭМ – на уровнях переопределения. 
...
```files
project/
    blocks/                # уровень переопределения
        header/
            header.css    
            header.js
        footer/
...
Пример файловой структуры БЭМ-проекта с тремя уровнями переопределения:

```files
project/
    common.blocks/                # уровень переопределения с базовыми блоками 
        header/
            header.css    
            header.js
        footer/
            footer.css    
            footer.js
    design.blocks/                # уровень переопределения с новым дизайном
        header/
            header.css    
        footer/
            footer.css    
...
* хранится на одном уровне переопределения  
  Это значит, что на уровне переопределения могут находиться готовые реализации БЭМ-сущностей. Например, уровнем переопределения с готовыми реализациями может быть библиотека блоков, подключенная в проект. (например, раелизация кнопки — ее внешний вид `button.css` и поведение `button.js`)
...
* добавление новых свойств блоку (доопределение)
* изменение существующих свойств блока (переопределение)
* подключение дополнительных БЭМ-сущностей в проект

### добавление новых свойств блоку
...
### изменение существующих свойств блока
...
### добавление новых готовых блоков в проект
