...
* хранится на одном уровне переопределения
  Это значит, что на уровне переопределения могут находиться готовые реализации БЭМ-сущностей. Например, уровнем переопределения с готовыми реализациями может быть библиотека блоков, подключенная в проект. (например, раелизация кнопки — ее внешний вид `button.css` и поведение `button.js`, `button.teml`)
...
project.blocks/         # базовые блоки проекта 
    ...
    footer/
        footer.css    
        footer.js
lib.blocks/             # блоки подключенной библиотеки
    button/
        button.css    
        button.js
        button.tmpl
...
* изменять старые свойства (переопределять)
* добавлять новые готовые блоки в проект
