...
В БЭМ-методологии блоки — это универсальные, повторно используемые компоненты страницы. Одни и те же блоки используются в разных проектах, однако это не означает, что все БЭМ-проекты одинаковые. Блоки можно изменять различными способами:
...
Изменение блока любым из перечисленных в выше способов не требует:
...
Используется, если важно только наличие или отсутствие модификатора у блока, а его значение несущественно. Например, модификатор, описывающий состояние «отключен»: `disabled`:
...
Создавайте микс, если нужная вам реализация требуется только для данного окружения и в данном виде точно не будет использоваться в проекте повторно.
...
* [одинаково форматировать разные HTML-элементы](#Стилизация-групп-блоков)
...
Иногда необходимо применить одинаковое форматирование сразу к нескольким различным HTML-элементам страницы. Обычно для решения подобных задач применяют групповые селекторы, которые позволяют быстро изменить дизайн страницы. Однако групповые селекторы увеличивают связанность кода.
...
Создавайте изменения на другом уровне переопределения, если:

* Нужная вам реализация может использоваться повторно в различных сборках проекта.  
