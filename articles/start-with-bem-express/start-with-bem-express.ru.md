...
Такое различие обусловлено описанием в `bem-express` [зависимостей](https://ru.bem.info/platform/deps/), для блоков, которые не отражены в декларации. Блок `root` является некой отправной точкой для сборщика.
...
Прежде чем мы начнем писать код, необходимо определить из каких основных блоков будет состоять проект.
